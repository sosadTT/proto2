...
class ImageSubscriber(Node):
    ...
    def __init__(self):
        ...
        # 비디오 라이터 초기화
        self.video_writer = cv2.VideoWriter('output.avi', cv2.VideoWriter_fourcc(*'XVID'), 10, (640, 480))
    ...
    def img_callback(self, data):
        ...
        # process_frame 함수 호출 시 error 값을 전달
    	result = process_frame(gray, self.error)
...
def process_frame(frame):
	...
	# 기존의 프레임 처리 로직...
    # 화면에 error 값을 표시
    cv2.putText(frame, f"Error: {error}", (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
    return frame
